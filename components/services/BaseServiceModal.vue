<template>
  <div v-if="modelValue" class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
    <!-- Background overlay -->
    <div class="fixed inset-0 bg-neutral-600/70 dark:bg-neutral-900/70 transition-opacity duration-500" aria-hidden="true" @click="$emit('close')"></div>

    <!-- Modal panel -->
    <div class="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
      <div class="relative transform overflow-hidden rounded-lg bg-white dark:bg-gray-800 px-4 pb-4 pt-5 text-left shadow-xl transition-all duration-500 sm:my-8 sm:w-full sm:max-w-lg sm:p-6">
        <!-- Close button -->
